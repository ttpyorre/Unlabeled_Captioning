...
class G_NET1(nn.Module):
    #Modified GNET without CNET
    def __init__(self):
...
#################################
class encoder_resnet1(nn.Module):
    def __init__(self):
...
class MAP_NET_TI(nn.Module):
    #Generator from text embedding to Image Embedding####
    def __init__(self):
...
class MAP_NET_TI2(nn.Module):
    ...
    def define_module(self):
        ...
        ##self.fc1 = nn.Sequential(
          ##  nn.Linear(ndf, (ndf + efg)//2),
            #nn.BatchNorm1d(ndf//2),
            #Swish(),
            ##nn.LeakyReLU(0.2, inplace=True))
            #nn.Linear(efg*4, efg*4),
            #nn.BatchNorm1d(efg*8),
            #Swish(),
            #nn.LeakyReLU(0.2, inplace=True))
            #nn.Linear(efg*8, efg*4))#,
            #nn.BatchNorm1d(efg*4),
            #Swish(),
            #nn.LeakyReLU(0.2, inplace=True))
...
class MAP_NET_IT22(nn.Module):
    #Generator from Image Embedding to text embedding####
    def __init__(self):
        super(MAP_NET_IT22, self).__init__()
        #self.df_dim = cfg.TEXT.DIMENSION
        self.ef_dim = cfg.TEXT.EMBEDDING_DIM #100
        self.in_dim =  cfg.GAN.EMBEDDING_DIM #128
        self.define_module()
...
class MAP_NET_TI22(nn.Module):
    #Generator from text embedding to Image Embedding####
    def __init__(self):
        super(MAP_NET_TI22, self).__init__()
        #self.df_dim = cfg.TEXT.DIMENSION
        self.ef_dim = cfg.GAN.EMBEDDING_DIM #128
        self.in_dim = cfg.TEXT.EMBEDDING_DIM #100
...
class MAP_NET_TI3(nn.Module):
    #Generator from text embedding to Image Embedding####
    def __init__(self):
        super(MAP_NET_TI3, self).__init__()
        #self.df_dim = cfg.TEXT.DIMENSION
        self.ef_dim = cfg.GAN.EMBEDDING_DIM #128
        self.in_dim = cfg.TEXT.EMBEDDING_DIM #100
    ...
    def define_module(self):
        ...
        self.fc1 = nn.Sequential(
            nn.Linear(ndf, (ndf + efg)//2),
            #nn.Linear(ndf, efg*4), #100 to 512
            #nn.BatchNorm1d(ndf//2),
            #Swish(),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Linear((ndf + efg)//2, (ndf + efg)//2),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Linear((ndf + efg)//2, (ndf + efg)//2),
            nn.LeakyReLU(0.2, inplace=True))
            #nn.Linear(efg*4, efg*4),
            ...
            #nn.LeakyReLU(0.2, inplace=True))
        self.fc2 = nn.Sequential(
                nn.Linear((ndf + efg)//2, efg, bias=True))
                #nn.Linear(ndf//8, efg),
                #nn.LeakyReLU(0.2, inplace=True))
                #nn.Tanh())
...
class MAP_NET_IT3(nn.Module):
    #Generator from Image Embedding to text embedding####
    def __init__(self):
        super(MAP_NET_IT3, self).__init__()
        #self.df_dim = cfg.TEXT.DIMENSION
        self.ef_dim = cfg.TEXT.EMBEDDING_DIM #100
        self.in_dim =  cfg.GAN.EMBEDDING_DIM #128
        self.define_module()
...
class D_NET_TEXT1(nn.Module):
#To discriminate for text embedding#######################3
    def __init__(self):
        super(D_NET_TEXT1, self).__init__()
        #self.ef_dim = cfg.GAN.EMBEDDING_DIM #128
        self.in_dim = cfg.TEXT.EMBEDDING_DIM #100
        self.define_module()
...
class D_NET_IMAGE1(nn.Module):
#To discriminate for text embedding#######################3
    def __init__(self):
        super(D_NET_IMAGE1, self).__init__()
        #self.df_dim = cfg.TEXT.DIMENSION
        self.ef_dim = cfg.GAN.EMBEDDING_DIM #128
        #self.in_dim = cfg.TEXT.EMBEDDING_DIM #100
